...
function validateDishes(dishes, next) {
    if (dishes) {
        if (dishes !== "") {
            if (dishes.length !== 0) {
...
function updateValidation(id, deliverTo, mobileNumber, status, dishes, next, req) {
    if (!id) {
        validateDeliverTo(deliverTo,dishes,next,status,mobileNumber)
    }
    else {
...
function validateFieldsForUpdate(req, res, next) {
    ...
    if (id !== req.params.id) {
        updateValidation(id, deliverTo, mobileNumber, status, dishes, next, req)
...
function validateDeliverTo(deliverTo,dishes,next,status,mobileNumber){
    if (deliverTo) {
        validateMobileNo(dishes,next,status,mobileNumber)
    }
    else {
        return next({ status: 400, message: "Order must include a deliverTo" })
    }
...
function validateMobileNo(dishes,next,status,mobileNumber){
    if (mobileNumber) {
        validateStatus(dishes,next,status)
    }
    else {
        return next({ status: 400, message: "Order must include a mobileNumber" })
    }
...
function validateStatus(dishes,next,status){
    if (status) {
        if (status !== "invalid") {
            validateDishes(dishes, next)
